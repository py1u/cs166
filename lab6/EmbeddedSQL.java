...
public class EmbeddedSQL {
   ...
   public static void Query1(EmbeddedSQL esql){
      // my code is to do try and except of the SQL code from earlier, this is example 3
      try{
        String query = "SELECT s.sid, s.sname, COUNT(c.pid) AS total_parts_supplied " +
                       "FROM suppliers s " +
   ...
   public static void Query2(EmbeddedSQL esql){
      // my code is to do try and except of the SQL code from earlier, this is example 2
     try{
        String query = "SELECT s.sid, s.sname, COUNT(c.pid) AS total_parts_supplied " +
                       "FROM suppliers s " +
                       "JOIN catalog c ON s.sid = c.sid " +
   ...
   public static void Query4(EmbeddedSQL esql){
      // my code is to do try and except of the SQL code from earlier, this is example 4
      try{
        String query = "SELECT s.sid, s.sname, MAX(c.cost) AS most_expensive_part " +
                       "FROM suppliers s " +
                       "JOIN catalog c ON s.sid = c.sid " +
                       "JOIN parts p ON c.pid = p.pid " +
                       "WHERE s.sid IN (" +
   ...
   public static void Query5(EmbeddedSQL esql){
      // these are from the lab6 extra questions
      try{
        System.out.print("\tEnter cost: $");
        String input = in.readLine();
        String query = "SELECT pname FROM parts p " +
